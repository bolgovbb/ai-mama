...
# ─────────────────────────────────────────────
# АУДИОПОДКАСТ (ElevenLabs TTS)
# ─────────────────────────────────────────────

PODCAST_SCRIPT_PROMPT = """Ты — редактор подкастов для проекта AI Mama (mama.kindar.app).
Перепиши текст статьи в разговорный подкаст-скрипт на русском языке.

Правила:
• Тёплый, дружелюбный тон — как будто подруга рассказывает
• Убери таблицы, цифры-ссылки, академические отсылки
• Начни: «Привет! Я Маша, и сегодня мы поговорим о...»
• Длина: 700–900 слов (~5 минут чтения вслух)
• Заверши: «Если хочется узнать больше — полный текст статьи ниже. До встречи!»
• Выведи ТОЛЬКО текст скрипта, без ремарок и пояснений"""


def generate_podcast(article_id: str, title: str, body_md: str, api_key: str) -> Optional[str]:
    """Генерирует аудиоподкаст: Claude скрипт → ElevenLabs TTS → upload."""
    if not ELEVENLABS_API_KEY:
        print("  ⚠️  ELEVENLABS_API_KEY не задан — подкаст не сгенерирован")
        return None

    # Шаг 1: Генерация подкаст-скрипта через Claude
    print(f"  🎙️  Генерируем подкаст-скрипт: {title[:50]}...")
    try:
        script_response = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY).messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=4096,
            system=PODCAST_SCRIPT_PROMPT,
            messages=[{"role": "user", "content": f"Вот текст статьи:\n\n# {title}\n\n{body_md[:8000]}"}]
        )
        script_text = script_response.content[0].text
        print(f"       Скрипт готов ({len(script_text)} символов)")
    except Exception as e:
        print(f"  ⚠️  Ошибка генерации скрипта: {e}")
        return None
    ...
    try:
        ...
        if tts_response.status_code != 200:
            print(f"  ⚠️  ElevenLabs ошибка: {tts_response.status_code} {tts_response.text[:200]}")
            return None

        audio_bytes = tts_response.content
        size_mb = len(audio_bytes) / (1024 * 1024)
        print(f"       Аудио готово ({size_mb:.1f} MB)")
    except Exception as e:
        print(f"  ⚠️  TTS ошибка: {e}")
        return None

    # Шаг 3: Загрузка на платформу
    try:
        files = {"file": (f"{article_id}.mp3", audio_bytes, "audio/mpeg")}
        r = requests.post(
            f"{BASE_URL}/api/v1/articles/{article_id}/audio",
            files=files,
            headers={"Authorization": f"Bearer {api_key}"},
            timeout=60
        )
        if r.status_code == 200:
            ...
            print(f"  ✅ Подкаст загружен: {BASE_URL}{audio_url}")
            return audio_url
        else:
            print(f"  ⚠️  Upload ошибка: {r.status_code} {r.text[:200]}")
            return None
    except Exception as e:
        print(f"  ⚠️  Upload ошибка: {e}")
        return None
...
"""
    ...
    try:
        ...
        # Generate cover image + audio podcast
        if article_id:
            generate_cover_image(article_id, article["title"], tags, platform_key, slug)
            generate_podcast(article_id, article["title"], article["body_md"], platform_key)
...
def generate_audio_all():
    """Генерирует аудиоподкасты для всех статей без аудио."""
    state = load_state()
    staff_key = state.get("platform_api_key_editor")
    if not staff_key:
        print("✗ Нет staff api_key. Запусти --register-staff.")
    ...
    r = requests.get(f"{BASE_URL}/api/v1/articles", params={"limit": 100}, timeout=15)
    if r.status_code != 200:
        print(f"✗ Ошибка API: {r.status_code}")
        return

    articles = r.json().get("items", [])
    need_audio = [a for a in articles if not a.get("audio_url")]

    if not need_audio:
        print("  ✓ Все статьи с аудио")
        return

    print(f"\n🎙️  {len(need_audio)} статей без аудио\n")
    ...
    for slug in AGENTS:
        ...
        api_key = state.get(f"platform_api_key_{slug}")
        if agent_id and api_key:
            agent_keys[agent_id] = api_key

    for article in need_audio:
        article_id = article["id"]
        agent_id = article.get("agent_id", "")
        title = article["title"]
        body_md = article.get("body_md", "")
        api_key = agent_keys.get(agent_id, staff_key)

        print(f"  📝 {title[:60]}...")
        result = generate_podcast(article_id, title, body_md, api_key)
        if result:
            time.sleep(5)  # Rate limit: пауза между вызовами ElevenLabs

    print(f"\n✅ Генерация аудио завершена.")
...
def main():
    ...
    parser.add_argument("--generate-audio",  action="store_true", help="Сгенерировать аудиоподкасты для всех статей")
